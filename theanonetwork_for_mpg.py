...
def load_auto_data():
    ...
    f = file("autompg.data", "rb")
    opened_data_x = numpy.loadtxt(f, usecols=(1,2,3,4,5,6,7))
    f.close()
    f = file("autompg.data", "rb")
    opened_data_y = numpy.loadtxt(f, usecols=(0,1))
    f.close()
    
    #Create the training set
    train_set_x = opened_data_x[:300]
    train_set_x = theano.shared(train_set_x)
    train_set_y = opened_data_y[:300]
    train_set_y_formatted = []
    for row in train_set_y:
        train_set_y_formatted = numpy.append(train_set_y_formatted, row[0])
    train_set_y = train_set_y_formatted
    train_set_y = theano.shared(train_set_y)
    train_set_y = T.cast(train_set_y, 'int32')

    #Create the validation set
    valid_set_x = opened_data_x[301:350]
    valid_set_x = theano.shared(valid_set_x)
    valid_set_y = opened_data_y[301:350]
    valid_set_y_formatted = []
    for row in valid_set_y:
        valid_set_y_formatted = numpy.append(valid_set_y_formatted, row[0])
    valid_set_y = valid_set_y_formatted
    valid_set_y = theano.shared(valid_set_y)
    valid_set_y = T.cast(valid_set_y, 'int32')

    #Creat the test set
    test_set_x = opened_data_x[351:]
    test_set_x = theano.shared(test_set_x)
    test_set_y = opened_data_y[351:]
    test_set_y_formatted = []
    for row in test_set_y:
        test_set_y_formatted = numpy.append(test_set_y_formatted, row[0])
